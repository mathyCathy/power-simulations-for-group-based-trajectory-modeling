...
* The outcome distributions conditional on the class membership, $C_i = k$, are based on a linear mixed effects model with random intercepts for each participant: $$Y_{ij,k} = \beta_{0,k} + \beta_{0i,k} + \beta_{1,k} (\mathrm{time_{ij}}) + \epsilon_{ij,k},$$ for individuals $i=1,\dots,N$ and times $t_{ij}$ where each individual has a random effect $\beta_{0i,k} \sim N(0,\sigma_u^2)$ and $\epsilon_{ij,k} \sim N(0,\sigma_e^2)$ is random noise. 
